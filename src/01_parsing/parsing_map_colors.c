...
static int convert_rgb(char **rgb, char **splited_line)
{
    ...
    if (arr_len(rgb) != 3) // ~ We check if the array has 3 elements (r, g, b)
        return (-1);
    r = ft_atoi(rgb[0]);
    g = ft_atoi(rgb[1]);
    b = ft_atoi(rgb[2]);
    if (r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255) // ~ We check if the values are between 0 and 255
        return (-1);
    return (r << 24 | g << 16 | b << 8 | 255); // ~ We return the color in the ARGB format
}
...
int parse_color(char **splited_line)
{
    ...
    color = 0;
    if (count_commas(splited_line[1]) != 2) // ~ We count the commas in the line to check if there are 2
        return (print_message(PARSING_ERROR, RGB_ERROR), -1);
    trimmed_line = ft_strtrim(splited_line[1], "\n"); // ~ We trim the line to remove the newline character
    rgb = ft_split(trimmed_line, ','); // ~ We split the line by commas
    if (!rgb)
