...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/19 14:43:56 by abzaiz            #+#    #+#             */
...
static char	*ft_strchr12( char *s, int c)
{
	int	i;
...
static char	*read_line(int fd, char *buffer, char *saved)
{
	...
	while (newline_ptr == NULL && bytes_read > 0)
	{
		...
		buffer[bytes_read] = '\0';
		temp = saved;
		saved = ft_strjoin1(saved, buffer);
		free(temp);
		newline_ptr = ft_strchr12(saved, '\n');
	}
...
static char	*extract_line(char **saved, int i)
{
	...
	if ((*saved)[i] == '\n')
	{
		line = ft_substr1(*saved, 0, i + 1);
		temp = *saved;
		*saved = ft_strdup1(*saved + i + 1);
		free(temp);
		if ((*saved)[0] == '\0')
		{
			free(*saved);
