...
/*   Updated: 2025/03/19 14:31:02 by abzaiz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	validate_horizontal_borders(t_params *params)
{
	...
	{
		if (params->map[0][i] != '1' && !ft_isspace(params->map[0][i]))
			return (false);
		i++;
	}
	j = arr_len(params->map) - 1;
	i = 0;
	while (params->map[j][i])
	{
		if (params->map[j][i] != '1' && !ft_isspace(params->map[j][i]))
			return (false);
		i++;
	}
	return (true);
