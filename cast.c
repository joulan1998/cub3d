...
int distane_to_wall(float x1, float y1, float x2, float y2)
{
...
//     return(res);
// }
// void    cal_v_dist(t_root *root, t_ray *ray, float rayangle)
// {
//     float   xintercept;
//     float   yintercept;
//     float   facingRight;
...
t_ray *cast_ray(t_root *root, t_ray *ray, float rayangle, int stripid)
{
    ...
    // Calculate distances
    float horDistance = foundHWallHit ? distane_to_wall(*root->player->player_x, *root->player->player_y, horHitX, horHitY) : INT_MAX;
